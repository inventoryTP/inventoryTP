...
if page == "📊 วิเคราะห์ยอดขาย":
    ...
    if not df.empty:
        ...
        # ตารางสรุปสินค้า (รวมตามรหัสสินค้า - คงเดิม)
        ...
        # ตารางสรุปวันที่สั่งซื้อ (คงเดิม)
        ...
        # เก็บตัวแปรไว้ใช้ทำกราฟรายเดือน
        ...
        # --- ส่วนที่เพิ่มใหม่: กราฟยอดการสั่งซื้อรายเดือน (ดึงเลขเดือนตรงกลาง) ---
        st.divider()
        st.subheader("📈 สรุปจำนวนรายการที่สั่งซื้อรายเดือน (แยกตามเดือน)")
        
        try:
            # ฟังก์ชันดึงเลขเดือนจาก DD/MM/YYYY
            def extract_month(date_str):
                parts = str(date_str).split('/')
                if len(parts) >= 2:
                    return parts[1] # คืนค่าเลขตรงกลาง (เดือน)
                return "ไม่ระบุ"

            graph_monthly_data['เลขเดือน'] = graph_monthly_data[date_col].apply(extract_month)
            
            # รวมจำนวนรายการที่สั่งซื้อ ตามเลขเดือน
            monthly_chart = graph_monthly_data.groupby('เลขเดือน')['จำนวนรายการที่สั่งซื้อ'].sum().reset_index()
            
            # เรียงลำดับเดือน 01-12
            monthly_chart = monthly_chart.sort_values('เลขเดือน')
            
            # เปลี่ยนเลขเดือนเป็นชื่อเดือน (แบบง่าย)
            month_names = {
                "01": "มกราคม", "02": "กุมภาพันธ์", "03": "มีนาคม", "04": "เมษายน",
                "05": "พฤษภาคม", "06": "มิถุนายน", "07": "กรกฎาคม", "08": "สิงหาคม",
                "09": "กันยายน", "10": "ตุลาคม", "11": "พฤศจิกายน", "12": "ธันวาคม"
            }
            monthly_chart['ชื่อเดือน'] = monthly_chart['เลขเดือน'].map(month_names).fillna(monthly_chart['เลขเดือน'])

            # แสดงกราฟ
            st.bar_chart(data=monthly_chart.set_index('ชื่อเดือน')['จำนวนรายการที่สั่งซื้อ'])
            
        except Exception as e:
            st.info("ไม่สามารถแสดงกราฟรายเดือนได้ กรุณาตรวจสอบรูปแบบวันที่ใน Google Sheets (ควรเป็น 18/01/2025)")
