...
if page == "📊 วิเคราะห์ยอดขาย":
    ...
    if not df.empty:
        ...
        # เก็บข้อมูลไว้ทำกราฟ (ก่อน Reset Index)
        graph_monthly_data = summary_date.copy()
        
        # แสดงตาราง (ลบคอลัมน์ชั่วคราวออก)
        ...
        # --- แก้ไขจุดที่ 2: กราฟรายเดือน เรียง มกราคม - ธันวาคม ---
        st.divider()
        ...
        try:
            ...
            # สร้างตารางอ้างอิงเดือน 1-12 เพื่อบังคับการเรียงลำดับ
            ...
            # นำข้อมูลยอดขายมารวมกับตารางเดือน (เพื่อให้เรียงลำดับมกราคม-ธันวาคมแน่นอน)
            final_monthly = pd.merge(all_months, monthly_chart, on="เลขเดือน", how="left").fillna(0)

            # แสดงกราฟ
            st.bar_chart(data=final_monthly.set_index('ชื่อเดือน')['จำนวนรายการที่สั่งซื้อ'])
            
        except Exception as e:
            st.info("ไม่สามารถแสดงกราฟรายเดือนได้ กรุณาตรวจสอบรูปแบบวันที่")
